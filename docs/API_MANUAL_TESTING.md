...
| **Bonus: full-text search** | ADR-020c | Add a request `GET /bookmarks?search=guards` → matches a bookmark by its **notes**; `?search=\"twelve-factor\"` (quoted phrase) and `?search=postgres -manual` (exclusion) work; `?search=` alone → 400 |
