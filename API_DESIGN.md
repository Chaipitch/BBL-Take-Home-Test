...
1. **Auth errors: outage reported as "invalid token".** A first design mapped jose errors to 401; jose throws a *generic* `JOSEError` when the JWKS endpoint returns non-200, so an Auth0 outage would have logged users out. Found by reading jose's source; fixed by classifying explicit error codes (401 vs 503). [ADR-010]
2. **Test helper made negative tests use valid tokens.** `SignJWT` setters overwrote the "bad" claims; 4 rejection tests failed and one acceptance test passed for the wrong reason. Found on first run; payload now built as one object. Mutation testing then showed the RS256 pin was untested → added a PS256 case. [ADR-010]
3. **Prisma upsert assumed atomic.** With `update: {}` Prisma runs SELECT-then-INSERT; 9/10 parallel first sign-ins returned 500. Caught by a parallel e2e test, measured 171/200 failures, fixed with a non-empty update. [ADR-011]
...
5. **Filters treated `%` and `_` as wildcards.** Assumed Prisma escapes `contains` values; a test written to check that assumption showed `?name=%` matched every collection. Fixed with `common/filters.ts` (escapes `\`, `%`, `_`). [ADR-013]
...
9. **A seed that quietly lost data on re-run.** Re-seeding deleted and recreated the seed users, which cascaded away a share the real test user had granted — and the block that recreates it is skipped for users who already have data. Found by the idempotency test; the share is now upserted on every run. [ADR-016]
10. **A test that could not prove its own claim.** "Uses the full-text index" asserted on an `EXPLAIN` plan, but Postgres sequential-scans a small table by choice, so the assertion passed regardless. Rewritten to disable `enable_seqscan` and assert the index name, then verified by dropping the index (test fails) and recreating it. [ADR-020c]
